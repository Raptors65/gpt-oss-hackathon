...
def list_notes() -> str:
    ...
    return str(os.listdir(NOTES_DIR))

@function_tool
def read_note(note_name: str) -> str:
    """Returns the content in the given note."""

    with open(os.path.join(NOTES_DIR, note_name), encoding="utf-8") as note_file:
...
def update_note(note_name: str, new_content: str):
    ...
    with open(os.path.join(NOTES_DIR, note_name), mode="w", encoding="utf-8") as note_file:
